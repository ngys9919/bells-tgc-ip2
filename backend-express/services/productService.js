...
const geolocationData = require('../services/Geolocation');
  ...
  async function getProductByProductCodeID(id) {
    const product = await productData.getProductByProductCodeID(id);
    if (!product) {
  ...
  async function getProductByProductID(id) {
    const product = await productData.getProductByProductID(id);
    if (!product) {
      throw new Error('Product not found');
    }
    return product;
  ...
  async function getProductByProductData(sortBy, sortOrder, filterAIproducts) {
    const product = await productData.getProductByProductData(sortBy, sortOrder, filterAIproducts);
    if (!product) {
      throw new Error('Product not found');
    }
    return product;
  ...
  async function getProductByProductTitle(searchBy, filterAIproducts) {
    const product = await productData.getProductByProductTitle(searchBy, filterAIproducts);
    if (!product) {
      throw new Error('Product not found');
    }
    return product;
